...
switch ($action) {
	...
	case 'start_add_template':
		...
		if ($add) $tmpl->msg('done', '?show=styles&action=show_templates&style_id='.htmlspecialchars($style_id));
		else $tmpl->msg('error');
		break;
	case 'edit_template':
	...
	case 'start_edit_template':
		...
		if ($update) $tmpl->msg('done', '?show=styles&action=show_templates&style_id='.htmlspecialchars($style_id,1));
		else $tmpl->msg('error');
		break;
